...
public final class GuiPanelScrollbar {
    ...
    private static final Identifier HANDLE_TEXTURE = Identifier.fromNamespaceAndPath(
    ...
    public void render(GuiGraphicsExtractor guiGraphics, int leftPos, int topPos) {
        updateButtonVisibility();
        if (!isNeeded()) {
            return;
        }
        ...
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, HANDLE_TEXTURE, trackX, handleY, 0.0F, 0.0F,
                HANDLE_WIDTH, HANDLE_HEIGHT, HANDLE_WIDTH, HANDLE_HEIGHT);
    }
