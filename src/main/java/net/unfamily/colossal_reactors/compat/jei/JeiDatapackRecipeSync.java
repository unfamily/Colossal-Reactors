...
public final class JeiDatapackRecipeSync {
    ...
    public static void syncWhenWorldReady() {
        ...
        addIfFewerThanExpected(recipeManager, ElecCoilRecipeCategory.RECIPE_TYPE, ElecCoilLoader.getJeIDefinitions());
    ...
    private static <T> void addIfFewerThanExpected(IRecipeManager recipeManager, RecipeType<T> recipeType, List<T> recipes) {
        if (recipes.isEmpty()) {
            return;
        }
