...
public final class IceAndFireIntegration {
    ...
    public static void register() {
        try {
            ...
            net.unfamily.colossal_reactors.ColossalReactors.LOGGER.warn(
    ...
    // inside breathAttack(burnX, burnY, burnZ).
    // We return false (don't cancel) so the attack proceeds and shows visuals.
    // Energy is added when the dragon has been warming up long enough (burnProgress >= 40).
    // -----------------------------------------------------------------------
    private static final class FireBlockHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) {
            ...
            try {
                ...
                // breathAttack passes rodPos.getX()+0.5 etc. – floor gives the exact rod block
                ...
                // Only add energy once the dragon is fully breathing (burnProgress >= 40).
                // burnProgress is a public field in DragonBaseEntity.
                double burnProgress = ((Number) dragon.getClass().getField("burnProgress").get(dragon)).doubleValue();
                if (burnProgress < 40) return false; // let warmup proceed without adding energy yet

                int rf = Config.LIGHTNING_GENERATOR_DRAGON_RF.get();
                int added = gen.receiveDragonStrikeEnergy(rf);
                if (added > 0) gen.setChanged();

                // Return false: let breathAttack continue so visuals (lightning bolt, particles) are shown
                return false;
            } catch (Throwable t) {
                net.unfamily.colossal_reactors.ColossalReactors.LOGGER.trace(
    ...
    // -----------------------------------------------------------------------
    // ON_DRAGON_DAMAGE_BLOCK – fired inside destroyAreaBreath with a centre
    // that is randomly offset ±1.5 blocks from the actual target position.
    // We return true (cancel) when our rod or generator is within ±3 blocks of
    // the centre, protecting the whole setup from collateral dragon damage.
    // -----------------------------------------------------------------------
    private static final class DamageBlockHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) {
            ...
            try {
                ...
                // Check ±3 blocks around the centre for our rod+generator setup.
                // This accounts for the random ±1.5 block offset used in performNormalBreathAttack.
                for (BlockPos p : BlockPos.betweenClosed(center.offset(-3, -3, -3), center.offset(3, 3, 3))) {
                    if (level.getBlockState(p).is(ModBlocks.HIGH_POWER_LIGHTNING_ROD.get())
                        ...
                        return true; // cancel block destruction near our setup
                    }
                }
            } catch (Throwable t) {
                net.unfamily.colossal_reactors.ColossalReactors.LOGGER.trace(
    ...
    private static final class LureImpl implements DragonLureCallback {
        ...
        public void tick(Level level, BlockPos generatorPos, BlockPos rodPos) {
            ...
            try {
                ...
                for (Object dragon : dragons) {
                    ...
                    // Skip dragons already committed to an assembled forge
                ...
                net.unfamily.colossal_reactors.ColossalReactors.LOGGER.trace("IceAndFire lure: {}", t.getMessage());
            }
        }

        /** Line-of-sight check using NeoForge Entity API directly. */
        ...
        /** True if the given IAF/NeoForge BlockPos holds an assembled DragonForgeInputBlockEntity. */
